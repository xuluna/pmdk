...
static int obj_init(struct benchmark *bench, struct benchmark_args *args)
{
  ...
  if (alloc_size < ALLOC_MIN_SIZE) alloc_size = ALLOC_MIN_SIZE;
  ...
  if (ob->pa->use_random_size) {
    ...
    for (size_t i = 0; i < n_ops_total; i++) ob->sizes[i] = args->dsize;
  }
 ...
 */
static int obj_exit(struct benchmark *bench, struct benchmark_args *args)
{
 ...
 */
static int pmalloc_init(struct benchmark *bench, struct benchmark_args *args) { return obj_init(bench, args); }

/*
 * pmalloc_op -- actual benchmark operation. Performs the pmalloc allocations.
 */
static int pmalloc_op(struct benchmark *bench, struct operation_info *info)
{
  auto *ob = (struct obj_bench *) pmembench_get_priv(bench);
 ...
 */
static int pmix_worker_init(struct benchmark *bench, struct benchmark_args *args, struct worker_info *worker)
{
 ...
 */
static void pmix_worker_fini(struct benchmark *bench, struct benchmark_args *args, struct worker_info *worker)
{
 ...
 */
static void shuffle_objects(uint64_t *objects, size_t start, size_t nobjects, unsigned *seed)
{
  uint64_t tmp;
  size_t   dest;
 ...
 */
static int pmalloc_exit(struct benchmark *bench, struct benchmark_args *args)
{
  auto *ob = (struct obj_bench *) pmembench_get_priv(bench);
 ...
 */
static int pfree_init(struct benchmark *bench, struct benchmark_args *args)
{
 ...
 */
static int pfree_op(struct benchmark *bench, struct operation_info *info)
{
  auto *ob = (struct obj_bench *) pmembench_get_priv(bench);
...
void obj_pmalloc_constructor(void)
{
  ...
  pmalloc_clo[0].off  = clo_field_offset(struct prog_args, use_random_size);
  ...
  pmalloc_clo[1].def            = "1";
  pmalloc_clo[1].type_uint.size = clo_field_size(struct prog_args, minsize);
  ...
  pmalloc_info.name = "pmalloc", pmalloc_info.brief = "Benchmark for internal pmalloc() "
                                                      "operation";
